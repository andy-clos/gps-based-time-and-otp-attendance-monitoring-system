...
<body onload="initClock()">
    <div class="main-container d-flex">
        <div class="sidebar" id="side_nav">
            ...
            <ul class="list-unstyled px-2">
                ...
                <li class=" mb-4"><a href="attendanceInfo.php" class="text-decoration-none px-3 py-2 d-block"><i
                        ...
                        Attendance</a></li>
                <li class="active mb-4"><a href="leaveInfo.php" class="text-decoration-none px-3 py-2 d-block"><i
        ...
        <div class="content">
            ...
            <div class="container">
                <h2 class="fs-1 mb-5" style="display: flex; justify-content: space-between;">MANAGE LEAVES
                <a href ="allLeave.php" download="Leave" class="btn btn-secondary"><i class="fal fa-file-alt"></i> Generate Report</a>
                </h2>
                ...
                <div class="card shadow mb-5" style="border-color: black; border-width: 2px;">
                    <ul class="list-group list-group-flush">
                        ...
                        <table id="table" class="table table-hover" style="width: 100%; text-align:center;">
                            ...
                            <tbody>
                                <?php
                                    $query = "SELECT * FROM leave_apply, users_info WHERE leave_apply.usersId = users_info.usersId order by startDate";
                                    if($result = mysqli_query($conn, $query)){
                                      if(mysqli_num_rows($result)>0){
                                          while ($data = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                                ?>
                                <tr>
                                    <td data-label="User ID"><?php echo $data['usersId'];?></td>
                                    <td data-label="Name"><?php echo $data['usersName'];?></td>
                                    ...
                                    <td data-label="Action">
                                        <!--WHEN ADMIN CLICK EITHER APPROVE OR REJECT, THESE 2 BUTTONS WILL DISAPPEAR AND THE DELETE BUTTON WILL APPEAR TO REPLACE-->
                                        
                                        <button 
                                            <?php 
                                                if($data['statusL'] == "Approved" || $data['statusL'] == "Rejected") { ?>
                                                    style = "display: none;" 
                                         <?php } ?> type="button" class="approve btn btn-info" onclick="window.location.href='../php_functions/approve_leave.php?id=<?php echo $data['usersId']; ?>&lid=<?php echo $data['leaveId']; ?>'"><i class="fal fa-check"></i> APPROVE
                                        </button>
                                        ...
                                        <button  
                                            <?php
                                                if($data['statusL'] == "Approved" || $data['statusL'] == "Rejected") { ?>
                                                    style = "display: none;" 
                                         <?php } ?> type="button" class="reject btn btn-info" onclick="window.location.href='../php_functions/reject_leave.php?id=<?php echo $data['usersId']; ?>&lid=<?php echo $data['leaveId']; ?>'"><i class="fal fa-times"></i></i> REJECT
                                        </button>
                                        ...
                                        <button
                                            <?php  
                                                if($data['statusL'] != "Approved" && $data['statusL'] != "Rejected") { ?>
                                                    style = "display: none;" 
                                         <?php } ?> type="button" class="delete btn btn-danger" onclick="return confirm('Are you sure to delete?'), window.location.href='../php_functions/delete_leave_admin.php?id=<?php echo $data['usersId']; ?>&lid=<?php echo $data['leaveId']; ?>'"><i class="fal fa-trash"></i></i>
                                            DELETE</button>
                                    </td>
                                </tr>
                                <?php
